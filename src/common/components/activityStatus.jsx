...
const ActivityStatus = () => {
    ...
    const StatusItem = ({ icon: Icon, count, color, label, bgColor }) => (
        <div
            className="relative group flex flex-col items-center justify-center w-11 h-11 rounded-full transition-all mb-2 border border-transparent dark:border-gray-800"
        ...
        >
            ...
            <div className="absolute left-full ml-3 px-2 py-1 bg-gray-900 text-white text-[12px] rounded opacity-0 transition-opacity pointer-events-none whitespace-nowrap z-50">
                {label}: {count}
            </div>
        </div>
    );
